...
Building a secure Framework isn't just about API tokens; it's about the entire infrastructure. `Rust-YALC` implements a "Deny by Default" mentality across databases and networks.
...
`Rust-YALC` provides standard initialization scripts in `scripts/security/`:
- **Postgres (`init_postgres_least_privilege.sql`)**: Revokes public permissions and creates an application user that can only perform CRUD operations on the specific `yalc_db` tables. It cannot create or drop databases or roles.
- **MongoDB (`init_mongo_least_privilege.js`)**: Creates a user restricted with the `readWrite` role exclusively on the `yalc_db` database.
...
`Rust-YALC` exposes a fluent builder in the `HttpTransport` to enforce explicit domain allowlisting:

```rust
use yalc_transports::HttpTransport;
use axum::Router;
