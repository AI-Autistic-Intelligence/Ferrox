...
impl PasetoAuth {
    ...
    pub fn generate_token(&self, payload: &AuthPayload, duration: Duration) -> Result<String, AppError> {
        ...
        // Encrypt and sign
        pasetors::local::encrypt(&self.key, &claims, None, Some(b"yalc-auth-footer"))
            .map_err(|e| AppError::InternalServerError(Box::new(e)))
    }
...
pub fn setup() {
    println!("yalc-security initialized: Argon2 Hashing and PASETO Authentication ready.");
}
